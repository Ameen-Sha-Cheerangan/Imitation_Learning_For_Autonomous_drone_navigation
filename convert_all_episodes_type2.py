...
if __name__ == "__main__":
    # current directory has subfolders 1,2,3,...
    root = "."   # or r"C:\Users\ameen\Desktop\Drone Project\Gaze Tracking\Data to be processed"

    for name in sorted(os.listdir(root)):
        episode_dir = os.path.join(root, name)
        if not os.path.isdir(episode_dir):
            continue
        if not name.isdigit():
            continue  # skip non-episode folders

        print(f"\n=== Converting episode {name} ===")
        prepare_data(episode_dir)
